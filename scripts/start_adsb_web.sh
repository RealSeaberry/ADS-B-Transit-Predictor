...
WEB_HOST="${ADSB_WEB_HOST:-100.75.150.117}"
...
Starts the ADS-B receiver stack:
  1. Optionally attach an SDR from Windows to WSL with usbipd
  2. Optionally start a local ADS-B decoder
  3. Start ADS-B Transit Web UI

Environment overrides:
  ADSB_USB_BUSID=<busid>   USBIPD BUSID for SDR; required for manual WSL attach
  ADSB_USB_MATCH_REGEX=... usbipd list regex used for SDR auto-detection
