...
function overlappedLabelAlpha3d(ac, labelBox, selected, emergency) {
  if (state.viewMode !== "3d" || aircraftLabel3dMode() !== "fade" || !labelBox.overlaps || selected || ac.conflict || ac.has_event || emergency) return 1;
  const overlap = Math.max(0, Math.min(1, Number(labelBox.overlapRatio || 0)));
  const minAlpha = 0.26;
  const maxAlpha = 1.0;
  return maxAlpha - (maxAlpha - minAlpha) * overlap;
}

function aircraftLabel3dMode() {
  return state.data?.settings?.web?.aircraft_label_3d_mode === "avoid" ? "avoid" : "fade";
}
...
function predictAircraftPosition(ac, dtSec) {
  ...
  const correctedAltitudeFt = Number(ac.corrected_alt_asl);
  ...
  const alt = Number.isFinite(altitudeFt)
    ...
    : ac.altitude;
  const correctedAlt = Number.isFinite(correctedAltitudeFt)
    ? correctedAltitudeFt + (Number.isFinite(vsFpm) ? vsFpm * Math.max(0, dtSec) / 60 : 0)
    : ac.corrected_alt_asl;
  return { lat: dest.lat, lon: dest.lon, altitude: alt, corrected_alt_asl: correctedAlt };
}
...
function activeDisplayAircraft() {
  ...
  const displayAircraft = aircraft.map((ac) => {
    ...
    let correctedAltAsl = predicted.corrected_alt_asl;
    if (prev && Number.isFinite(prev.lat) && Number.isFinite(prev.lon)) {
      ...
      if (jumpKm !== null && jumpKm < 5) {
        ...
        if (Number.isFinite(Number(prev.corrected_alt_asl)) && Number.isFinite(Number(predicted.corrected_alt_asl))) {
          correctedAltAsl = prev.corrected_alt_asl + (predicted.corrected_alt_asl - prev.corrected_alt_asl) * alpha;
        }
      }
    }
    state.aircraftSmoothing.set(ac.icao, { lat, lon, altitude, corrected_alt_asl: correctedAltAsl, frameMs });
    const next = { ...ac, lat, lon, altitude, corrected_alt_asl: correctedAltAsl, client_extrapolated_sec: dt };
...
function aircraftAltitudeLabel(ac, showTrend = false) {
  const base = formatAltitudeFt(ac?.altitude, 0);
  const trend = showTrend ? verticalTrend(ac?.vs) : "";
  return `${base}${trend}`;
}
...
const aircraftLabelNames = {
  ...
  distance: "Distance",
  altitude: "Altitude",
  corrected_alt_asl: "Corrected alt ASL",
  altitude_factor: "Alt factor",
  altitude_offset: "Alt offset",
  speed: "Speed",
  track: "Track",
  vs: "V/S",
  squawk: "Squawk",
...
function aircraftLabelValue(ac, key) {
  const values = {
    ...
    altitude: aircraftAltitudeLabel(ac, true),
    corrected_alt_asl: `${formatAltitudeFt(ac.corrected_alt_asl, 0)}${verticalTrend(ac?.vs)}`,
    altitude_factor: ac.altitude_factor === null || ac.altitude_factor === undefined || Number.isNaN(Number(ac.altitude_factor)) ? "--" : Number(ac.altitude_factor).toFixed(4),
    altitude_offset: formatAltitudeFt(ac.altitude_offset, 0),
...
function aircraftScreenPlaneSortDistance(ac) {
  if (state.viewMode !== "3d") return Number(ac.view_distance_km ?? ac.distance_km ?? Infinity);
  const data = state.data;
  if (!data || ac?.lat === null || ac?.lon === null) return Infinity;
  const center = state.center || data.settings.center || data.settings.user;
  const latRad = (Math.PI / 180) * center.lat;
  const kmPerDegLat = 111.32;
  const kmPerDegLon = Math.max(8, 111.32 * Math.cos(latRad));
  const eastKm = (Number(ac.lon) - center.lon) * kmPerDegLon;
  const southKm = -(Number(ac.lat) - center.lat) * kmPerDegLat;
  const yaw = ((state.viewYaw || 0) * Math.PI) / 180;
  const pitch = clamp3dPitch(state.viewPitch ?? 55) * Math.PI / 180;
  const rotatedSouthKm = eastKm * Math.sin(yaw) + southKm * Math.cos(yaw);
  const altitudeKm = (aircraftAltM(ac) - userGroundAltitudeM(data)) / 1000;
  const depthKm = rotatedSouthKm * Math.sin(pitch) + altitudeKm * Math.cos(pitch);
  return -depthKm;
}
...
function altitudeTrailColor(altitudeFt, minFt = null, maxFt = null) {
  if (altitudeFt === null || altitudeFt === undefined) return "#9ca3af";
  const altitude = Math.max(0, Number(altitudeFt));
...
function povBoxCandidates(anchor, size, index, rect) {
  const margin = 8;
  const gap = 24 + 14 * (index % 3);
  const diagonal = 32 + 10 * (index % 2);
  const half = size / 2;
  return [
    { x: anchor.x + gap, y: anchor.y - half },
    { x: anchor.x - size - gap, y: anchor.y - half },
    { x: anchor.x - half, y: anchor.y + gap },
    { x: anchor.x - half, y: anchor.y - size - gap },
    { x: anchor.x + diagonal, y: anchor.y + diagonal },
    { x: anchor.x - size - diagonal, y: anchor.y + diagonal },
    { x: anchor.x + diagonal, y: anchor.y - size - diagonal },
    { x: anchor.x - size - diagonal, y: anchor.y - size - diagonal },
  ].map((box) => ({
...
function povPlacementKey(ev, index = 0) {
  const icaos = (ev.icaos || []).join("-");
  return `${ev.type}:${icaos || ev.callsigns?.join("-") || index}`;
}

function clampPovBox(box, size, rect) {
  const margin = 8;
  return {
    x: Math.max(margin, Math.min(rect.width - size - margin, Number(box?.x) || margin)),
    y: Math.max(margin, Math.min(rect.height - size - margin, Number(box?.y) || margin)),
    w: size,
    h: size,
  };
}
...
function choosePovBox(ev, anchor, size, index, occupied, rect, avoidSegments = []) {
  const key = povPlacementKey(ev, index);
  const prior = state.povPlacements.get(key);
  const candidates = povBoxCandidates(anchor, size, index, rect);
  if (prior) {
    const stablePrior = clampPovBox(prior, size, rect);
    candidates.unshift(stablePrior);
  }
  ...
  for (const candidate of candidates) {
    ...
    const linePenalty = anchorDistance / 90;
    const farPenalty = Math.max(0, anchorDistance - size * 1.65) * 2.2;
    const priorDistance = prior ? Math.hypot(candidate.x - prior.x, candidate.y - prior.y) : 0;
    const priorPenalty = prior ? Math.min(260, priorDistance / 3.5) : 0;
    const score = overlaps * 1600 + lineHits * 220 + linePenalty + farPenalty + priorPenalty;
    if (score < bestScore) {
      best = candidate;
      bestScore = score;
...
function drawEventPovPreview() {
  ...
  [...selectedEvents, ...otherEvents].slice(0, 8).forEach((ev, index) => {
    ...
    if (anchor) {
      visibleKeys.add(povPlacementKey(ev, index));
...
function labelOverlapRatio(box, boxes) {
  const area = Math.max(1, box.w * box.h);
  let overlapArea = 0;
  boxes.forEach((other) => {
    const w = Math.max(0, Math.min(box.x + box.w, other.x + other.w) - Math.max(box.x, other.x));
    const h = Math.max(0, Math.min(box.y + box.h, other.y + other.h) - Math.max(box.y, other.y));
    overlapArea += w * h;
  });
...
function placeLabel(anchor, width, height, boxes, key, metrics = aircraftLabelMetrics()) {
  ...
  const box = labelBoxForCandidate(anchor, width, height, fallback, metrics);
  box.overlaps = intersectsAnyLabel(box, boxes);
  box.overlapRatio = labelOverlapRatio(box, boxes);
  return box;
}
...
function drawAircraft(options = {}) {
  ...
  const orderedAircraft = [...aircraft].sort((a, b) => {
    ...
    return aircraftScreenPlaneSortDistance(a) - aircraftScreenPlaneSortDistance(b);
  });
  ...
  orderedAircraft.forEach((ac) => {
    ...
    const labelHeight = labelParts.length * labelMetrics.lineHeight;
    const use3dFade = state.viewMode === "3d" && aircraftLabel3dMode() === "fade";
    const labelBox = use3dFade
      ? labelBoxForCandidate(p, labelWidth, labelHeight, "rightTop", labelMetrics)
      : placeLabel(p, labelWidth, labelHeight, labelBoxes, labelKey, labelMetrics);
    if (use3dFade) {
      labelBox.overlaps = intersectsAnyLabel(labelBox, labelBoxes);
      labelBox.overlapRatio = labelOverlapRatio(labelBox, labelBoxes);
    }
    labelBoxes.push(labelBox);
    const labelAlpha = overlappedLabelAlpha3d(ac, labelBox, selected, emergency);
    ctx.save();
...
function draw3dView() {
  ...
  activeDisplayAircraft().forEach((ac) => {
    ...
    vctx.fillText(`${ac.callsign || ac.icao} ${aircraftAltitudeLabel(ac)}`, pa.x + 6, pa.y - 5);
  });
...
function drawArView() {
  ...
  activeDisplayAircraft().forEach((ac) => {
    ...
    vctx.fillStyle = color;
    vctx.font = "11px ui-monospace, monospace";
    vctx.fillText(`${ac.callsign || ac.icao} ${aircraftAltitudeLabel(ac)}`, p.x + 8, p.y - 5);
  });
}
...
function updateStatus() {
  ...
  if (els.activeCount) els.activeCount.textContent = data.counts.active_total;
...
function updateTraffic() {
  ...
  els.trafficList.innerHTML = aircraft.slice(0, 80).map((ac) => {
    ...
    return `<button class="traffic-row${active}" data-icao="${ac.icao}">
      ...
      <span>${aircraftAltitudeLabel(ac)}</span>
...
function geoidSettingsHtml(cfg, opt) {
  const localModels = Array.isArray(opt.geoid_models) ? opt.geoid_models : [];
  const status = state.data?.settings?.geoid || {};
  if (!localModels.length) {
    return `
      <input type="hidden" data-key="geoid_correction_enabled" value="false">
      <p class="settings-hint">EGM geoid correction is not installed. Place a supported GeographicLib .pgm grid in data/geoids to enable it.</p>
    `;
  }
  const current = localModels.includes(cfg.geoid_model) ? cfg.geoid_model : localModels[0];
  const choices = ["off", ...localModels];
  const selected = cfg.geoid_correction_enabled ? current : "off";
  const detail = status.loaded
    ? `Loaded ${status.model}`
    : (status.error || "Installed locally. Select a model to convert alt_geom from ellipsoid height to MSL.");
  return `
    <input type="hidden" data-key="geoid_correction_enabled" id="geoidCorrectionEnabled" value="${selected !== "off"}">
    <div class="field"><label for="cfg_geoid_model">EGM geoid correction</label><select id="cfg_geoid_model" data-key="geoid_model">
      ${choices.map((optName) => `<option value="${optName}" ${optName === selected ? "selected" : ""}>${optName === "off" ? "Off" : optName}</option>`).join("")}
    </select></div>
...
function sampleAircraftForPreview() {
  ...
  return aircraft.find((ac) => ac.callsign) || aircraft[0] || {
    ...
    track: 273,
    vs: 832,
    corrected_alt_asl: 18780,
    altitude_factor: 1.0097,
    altitude_offset: 180,
    squawk: "1200",
  };
}
  ...
  els.settingsBody.innerHTML = `
    ...
    <section class="settings-group">
      ...
      <div class="settings-grid">
        ...
        ${selectField("web.aircraft_label_3d_mode", "3D label behavior", web.aircraft_label_3d_mode || "fade", opt.aircraft_label_3d_modes)}
    ...
    <section class="settings-group full">
      <h2>Altitude Correction</h2>
      <p class="settings-hint">Corrects ADS-B barometric altitude to geometric altitude, improving Sun/Moon transit prediction accuracy.</p>
      <div class="check-list">
        <label><input type="checkbox" id="altCorrEnabled" ${cfg.alt_correction_mode !== "none" ? "checked" : ""}> Enable altitude correction</label>
      </div>
      <input type="hidden" data-key="alt_correction_mode" id="altCorrMode" value="${cfg.alt_correction_mode || "none"}">
      <div id="altCorrBody" style="display:${cfg.alt_correction_mode !== "none" ? "block" : "none"};margin-top:10px;padding-left:14px;border-left:2px solid #444">
        <div class="check-list">
          <label><input type="radio" name="alt_corr_method" id="altCorrMetarRadio" value="metar" ${cfg.alt_correction_mode === "metar" ? "checked" : ""}> METAR auto-correction <small style="color:#888">(requires internet)</small></label>
          <label><input type="radio" name="alt_corr_method" id="altCorrManualRadio" value="manual" ${cfg.alt_correction_mode === "manual" ? "checked" : ""}> Manual input</label>
        </div>
        <div id="altCorrMetarPanel" style="display:${cfg.alt_correction_mode === "metar" ? "block" : "none"};margin-top:8px">
          <div id="metarDisplay" style="font-size:0.83em;padding:8px 10px;background:#111;border-radius:4px;color:#bbb;min-height:36px;line-height:1.6">Loading METAR…</div>
          <button type="button" id="metarTestBtn" style="margin-top:6px;font-size:0.85em;padding:3px 10px">Test Connection</button>
        </div>
        <div id="altCorrManualPanel" style="display:${cfg.alt_correction_mode === "manual" ? "block" : "none"};margin-top:8px">
          <div class="settings-grid">
            ${field("alt_correction_manual_temp_c", "Temperature °C", cfg.alt_correction_manual_temp_c ?? 15, "number", "0.1")}
            ${field("alt_correction_manual_qnh_hpa", "QNH hPa", cfg.alt_correction_manual_qnh_hpa ?? 1013, "number", "0.1")}
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="settings-group full">
      <h2>Dev</h2>
      <p class="settings-hint">Experimental and unstable options. These can reduce altitude stability and prediction reliability.</p>
      <div class="check-list">
        ${checkbox("web.show_dev_options", "Enable dev options", web.show_dev_options)}
      </div>
      <div id="devOptionsBody" style="display:${web.show_dev_options ? "block" : "none"};margin-top:10px;padding-left:14px;border-left:2px solid rgba(150,150,150,0.35)">
        <div class="check-list">
          ${checkbox("gps_altitude_correction_enabled", "GPS altitude correction (requires dump1090-fa/readsb JSON)", cfg.gps_altitude_correction_enabled)}
        </div>
        ${geoidSettingsHtml(cfg, opt)}
      </div>
    </section>
  `;
...
function bindAltCorrUI() {
  const devToggle = els.settingsBody?.querySelector('[data-key="web.show_dev_options"]');
  const devBody = document.getElementById("devOptionsBody");
  devToggle?.addEventListener("change", () => {
    if (devBody) devBody.style.display = devToggle.checked ? "block" : "none";
  });

  const enabled    = document.getElementById("altCorrEnabled");
  const body       = document.getElementById("altCorrBody");
  const modeInput  = document.getElementById("altCorrMode");
  const metarRadio = document.getElementById("altCorrMetarRadio");
  const manualRadio= document.getElementById("altCorrManualRadio");
  const metarPanel = document.getElementById("altCorrMetarPanel");
  const manualPanel= document.getElementById("altCorrManualPanel");
  if (!enabled) return;
  ...
  function updateMode() {
    ...
    const checkedRadio = document.querySelector('input[name="alt_corr_method"]:checked');
    const method = checkedRadio?.value || "metar";
    if (!checkedRadio && metarRadio) metarRadio.checked = true;
    modeInput.value = method;
    metarPanel.style.display = method === "metar" ? "block" : "none";
    manualPanel.style.display = method === "manual" ? "block" : "none";
    if (method === "metar") loadMetarDisplay();
  }

  enabled.addEventListener("change", updateMode);
  metarRadio?.addEventListener("change", updateMode);
  manualRadio?.addEventListener("change", updateMode);
  document.getElementById("metarTestBtn")?.addEventListener("click", testMetarConnection);
  document.getElementById("cfg_geoid_model")?.addEventListener("change", (event) => {
    const hidden = document.getElementById("geoidCorrectionEnabled");
    if (hidden) hidden.value = String(event.target.value !== "off");
  });

  if (enabled.checked && metarRadio?.checked) loadMetarDisplay();
}
...
function readSettingsForm() {
  ...
  els.settingsBody.querySelectorAll("[data-key]").forEach((node) => {
    ...
    if (cleanKey === "geoid_model" && node.value === "off") {
      target.geoid_correction_enabled = false;
      return;
    }
    if (node.type === "checkbox") target[cleanKey] = node.checked;
    else if (node.type === "hidden" && ["true", "false"].includes(String(node.value))) target[cleanKey] = node.value === "true";
...
async function saveSettings() {
  ...
  // Sync form UI state with what the server actually saved
  const savedMode = payload.config?.alt_correction_mode;
  if (savedMode !== undefined) {
    const modeInput  = document.getElementById("altCorrMode");
    const metarR     = document.getElementById("altCorrMetarRadio");
    const manualR    = document.getElementById("altCorrManualRadio");
    const enabledCb  = document.getElementById("altCorrEnabled");
    const altBody    = document.getElementById("altCorrBody");
    const metarPanel = document.getElementById("altCorrMetarPanel");
    const manualPanel= document.getElementById("altCorrManualPanel");
    if (modeInput)   modeInput.value    = savedMode;
    if (metarR)      metarR.checked     = savedMode === "metar";
    if (manualR)     manualR.checked    = savedMode === "manual";
    if (enabledCb)   enabledCb.checked  = savedMode !== "none";
    if (altBody)     altBody.style.display    = savedMode !== "none" ? "block" : "none";
    if (metarPanel)  metarPanel.style.display  = savedMode === "metar"  ? "block" : "none";
    if (manualPanel) manualPanel.style.display = savedMode === "manual" ? "block" : "none";
...
function useGps() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (error) => {
      els.settingsMessage.textContent = error.message || "GPS request failed.";
    },
...
async function fetchState(options = {}) {
  let detail = options.detail === "light" ? "light" : "full";
  const rect = canvas.getBoundingClientRect();
  if (state.fetchTimer) {
    clearTimeout(state.fetchTimer);
    state.fetchTimer = null;
  }
  const interacting = isMapInteracting();
  if (interacting && detail === "full" && !options.force) {
    state.pendingFetch = true;
    state.pendingFetchDetail = "full";
    detail = "light";
  }
...
// Prevent Enter key inside settings form from closing the dialog without saving
